...
import 'package:movie_tickets_booking/controllers/common_controller.dart';
...
import 'package:movie_tickets_booking/widgets/item_block.dart';
...
class _ViewAllScreenState extends State<ViewAllScreen> {
  ...
  @override
  void initState() {
    if (menu.name.contains("Sự kiện")) {
      list = events;
    } else if (menu.name.contains("Phim")) {
      list = movies;
    } else {
      list = plays;
    }
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      ...
      appBar: AppBar(
       title: Text(
        "${menu.name} ở ${LocationController.instance.city}",
         maxLines: 1,
  ...
  iconTheme: IconThemeData(
    color: Colors.white, // Thiết lập màu của mũi tên quay về thành màu trắng
  ),
),
